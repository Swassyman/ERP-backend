...
CREATE UNIQUE INDEX "role_code_index" ON "role" USING btree ("code") WHERE "role"."deleted_at" is null;--> statement-breakpoint
CREATE UNIQUE INDEX "user_email_index" ON "user" USING btree ("email") WHERE "user"."deleted_at" is null;--> statement-breakpoint
CREATE UNIQUE INDEX "user_role_user_id_role_id_managed_entity_id_index" ON "user_role" USING btree ("user_id","role_id","managed_entity_id") WHERE "user_role"."deleted_at" is null;